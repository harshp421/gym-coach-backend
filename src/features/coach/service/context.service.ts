 ...
 */
export async function buildSystemPrompt(userId: string, name: string | null): Promise<string> {
    ...
    lines.push(
        `You are an AI gym coach for ${who}. Be concise and direct. Don't quote the prescription back at them — they can see it. Focus on what to do next.`,
        "",
    )
    ...
    if (profile) {
        lines.push("Profile:")
        lines.push(
            `- Goal: ${profile.goal ?? "—"}${profile.targetWeightKg ? ` (target ${profile.targetWeightKg}kg)` : ""}`,
        )
        lines.push(
            `- Level: ${profile.experienceLevel ?? "—"}, ${profile.trainingDaysPerWeek ?? "—"}/wk`,
        )
    ...
    if (plan) {
        ...
        lines.push(
            `Active plan${planName}: ${split} (${plan.daysPerWeek} days/wk)`,
        )
        lines.push("Days:")
        for (const day of plan.days) {
            ...
            const exes = day.exercises
                .map(
                    (pe) =>
                        `${pe.exercise.name} ${pe.targetSets}×${pe.targetRepsMin}–${pe.targetRepsMax}`,
                )
                .join("; ")
        ...
        for (const s of recent.items) {
            ...
            const vol =
                s.summary.totalVolumeKg > 0
                    ...
                    : ""
            lines.push(
                `- ${fmtDate(s.startedAt)}: ${s.summary.totalSets} sets${vol}${status}`,
            )
        }
        lines.push("")
    }
    ...
    if (metrics.length > 0) {
        ...
        for (const m of metrics) {
            ...
            if (m.waistCm !== null) parts.push(`${m.waistCm}cm waist`)
    ...
    if (customs.length > 0) {
        ...
        lines.push(
            `User's custom exercises (${customs.length}): ${names}. Use those names exactly when referring to them.`,
        )
        lines.push("")
    }

    lines.push(
        "If you suggest a plan tweak, give the change clearly enough that the user can apply it themselves in the plan editor. Don't format the response as a template — just answer.",
    )

    return lines.join("\n")
}

// Bring back recent body metrics here without round-tripping the
// profile-routes layer; profile.service.listBodyMetrics is public-ish.
type BodyMetricMini = {
    recordedAt: string
...
async function listRecentBodyMetrics(
    userId: string,
    limit: number,
): Promise<BodyMetricMini[]> {
    const r = await pool.query(
        `SELECT recorded_at, weight_kg, body_fat_pct, waist_cm
         FROM body_metrics
         WHERE user_id = $1
