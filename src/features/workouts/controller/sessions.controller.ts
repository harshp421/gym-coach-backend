...
export async function logSet(req: Request, res: Response) {
    const setLog = await sessionsService.upsertSetLog(
        userId(req),
        paramId(req, "id"),
        req.body
    )
...
export async function deleteSet(req: Request, res: Response) {
    await sessionsService.deleteSetLog(
        userId(req),
        paramId(req, "id"),
        paramId(req, "setLogId")
    )
    res.status(204).send()
}

// GET /api/v1/workouts/sessions?limit=N&before=ISO
export async function listRecentSessions(req: Request, res: Response) {
...
export async function getExerciseHistory(req: Request, res: Response) {
    ...
    const result = await historyService.getExerciseHistory(
        userId(req),
