...
async function loadSetsForSession(sessionId: string): Promise<SetLog[]> {
    const result = await pool.query(
        `SELECT * FROM set_logs
         WHERE session_id = $1
         ORDER BY plan_exercise_id, set_number ASC`,
        [sessionId]
    )
...
export async function createSession(
    ...
    const dayResult = await pool.query(
        `SELECT pd.id, pd.plan_id
         FROM plan_days pd
         JOIN workout_plans wp ON wp.id = pd.plan_id
         WHERE pd.id = $1 AND wp.user_id = $2`,
        [planDayId, userId]
    )
    if (!dayResult.rows[0]) throw new HttpError(404, "Plan day not found")
    const planId: string = dayResult.rows[0].plan_id

    try {
        const result = await pool.query(
            `INSERT INTO workout_sessions (user_id, plan_id, plan_day_id)
             VALUES ($1, $2, $3)
             RETURNING *`,
            [userId, planId, planDayId]
        )
        return mapSession(result.rows[0])
    } catch (err: any) {
        ...
        if (err?.code === "23505") {
            throw new HttpError(
                409,
                "You already have a workout in progress. Finish or abandon it first.",
            )
        }
        throw err
    }
...
export async function upsertSetLog(
    ...
    const fitResult = await pool.query(
        `SELECT 1 FROM plan_exercises
         WHERE id = $1 AND plan_day_id = $2`,
        [input.planExerciseId, session.planDayId]
    )
    if (!fitResult.rows[0]) {
...
export async function deleteSetLog(
    ...
    const result = await pool.query(
        `DELETE FROM set_logs WHERE id = $1 AND session_id = $2`,
        [setLogId, sessionId]
    )
