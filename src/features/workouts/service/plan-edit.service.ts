...
import type {
    ...
    UpdateExerciseInput,
    UpdatePlanInput,
} from "../plan-edit.schemas.js"

// ---------------------------------------------------------------------------
// Internal helpers — every mutation runs through these.
// ---------------------------------------------------------------------------
 ...
 */
async function assertNoActiveSession(client: Client, userId: string) {
...
async function getActivePlanRow(
    client: Client,
    userId: string,
): Promise<{ id: string; splitType: string }> {
    const r = await client.query(
        `SELECT id, split_type FROM workout_plans
         WHERE user_id = $1 AND status = 'active'
         LIMIT 1`,
        [userId],
    )
    if (!r.rows[0]) throw new HttpError(404, "No active plan")
    return { id: r.rows[0].id, splitType: r.rows[0].split_type }
}

/** Auto-promote to `'custom'` on the first non-swap edit. */
async function flipToCustomIfNeeded(
    client: Client,
    planId: string,
    currentSplit: string,
) {
    if (currentSplit === "custom") return
    await client.query(
        `UPDATE workout_plans SET split_type = 'custom' WHERE id = $1`,
        [planId],
    )
}
 ...
 */
function translateFk(err: unknown): never {
    if (
        err &&
        typeof err === "object" &&
        "code" in err &&
...
async function getOwnedDay(
    ...
    const r = await client.query(
        `SELECT pd.plan_id
         ...
         WHERE pd.id = $1 AND wp.user_id = $2 AND wp.status = 'active'`,
        [dayId, userId],
    )
    if (!r.rows[0]) throw new HttpError(404, "Day not found")
...
async function getOwnedPlanExercise(
    ...
    const r = await client.query(
        `SELECT pd.plan_id, pe.plan_day_id
         FROM plan_exercises pe
         JOIN plan_days pd ON pd.id = pe.plan_day_id
         JOIN workout_plans wp ON wp.id = pd.plan_id
         WHERE pe.id = $1 AND wp.user_id = $2 AND wp.status = 'active'`,
        [planExerciseId, userId],
    )
    if (!r.rows[0]) throw new HttpError(404, "Plan exercise not found")
...
async function loadActivePlanOrThrow(userId: string): Promise<WorkoutPlan> {
    const plan = await getActivePlan(userId)
    if (!plan) throw new HttpError(500, "Plan disappeared during write")
    return plan
}
...
export async function updatePlan(
    ...
    await tx(async (c) => {
        ...
        values.push(plan.id)
        await c.query(
            `UPDATE workout_plans SET ${sets.join(", ")} WHERE id = $${i}`,
            values,
        )
...
export async function updateDay(
    ...
    await tx(async (c) => {
        ...
        values.push(dayId)
        await c.query(
            `UPDATE plan_days SET ${sets.join(", ")} WHERE id = $${i}`,
            values,
        )
        await flipToCustomIfNeeded(c, plan.id, plan.splitType)
    })
...
export async function deleteDay(
    ...
    await tx(async (c) => {
        ...
        const count = await c.query(
            `SELECT COUNT(*)::int AS n FROM plan_days WHERE plan_id = $1`,
            [plan.id],
        )
...
export async function reorderDays(
    ...
    await tx(async (c) => {
        ...
        const existing = await c.query(
            `SELECT id FROM plan_days WHERE plan_id = $1`,
            [plan.id],
        )
        ...
        for (let i = 0; i < indices.length; i++) {
            if (indices[i] !== i) {
                throw new HttpError(
                    400,
                    "dayIndex must be 0..N-1 with no gaps",
                )
            }
        }
        ...
        await c.query(
            `UPDATE plan_days SET day_index = day_index + 1000 WHERE plan_id = $1`,
            [plan.id],
        )
        for (const item of input) {
            await c.query(
                `UPDATE plan_days SET day_index = $1 WHERE id = $2`,
                [item.dayIndex, item.dayId],
            )
        }
        await flipToCustomIfNeeded(c, plan.id, plan.splitType)
    })
...
export async function createExercise(
    ...
    await tx(async (c) => {
        ...
        if (input.exerciseId) {
            ...
            if (!r.rows[0]) throw new HttpError(404, "Exercise not found")
        } else if (input.userExerciseId) {
            const r = await c.query(
                `SELECT 1 FROM user_exercises
                 WHERE id = $1 AND user_id = $2 AND archived_at IS NULL`,
                [input.userExerciseId, userId],
            )
            if (!r.rows[0])
...
export async function updateExercise(
    ...
    await tx(async (c) => {
        ...
        if (input.exerciseId !== undefined) {
            ...
            if (!ex.rows[0]) throw new HttpError(404, "Exercise not found")
        ...
        if (input.targetSets !== undefined) setIf("target_sets", input.targetSets)
        ...
        if (input.targetRpe !== undefined) setIf("target_rpe", input.targetRpe)
        if (input.restSeconds !== undefined)
            setIf("rest_seconds", input.restSeconds)
        if (input.notes !== undefined) setIf("notes", input.notes)

        if (sets.length === 0) return
        values.push(planExerciseId)
        try {
            await c.query(
                `UPDATE plan_exercises SET ${sets.join(", ")} WHERE id = $${i}`,
                values,
            )
        } catch (err: any) {
            if (err?.code === "23514") {
        ...
        const editedNonSwap = Object.keys(input).some(
            (k) =>
                k !== "exerciseId" &&
                k !== "userExerciseId" &&
...
export async function reorderExercises(
    ...
    await tx(async (c) => {
        ...
        const existing = await c.query(
            `SELECT id FROM plan_exercises WHERE plan_day_id = $1`,
            [dayId],
        )
        const existingIds = new Set(existing.rows.map((r) => r.id))
        if (existingIds.size !== input.length) {
        ...
        for (const item of input) {
            if (!existingIds.has(item.planExerciseId)) {
                throw new HttpError(
                    400,
                    `Unknown exercise ${item.planExerciseId}`,
                )
            }
        ...
        for (let i = 0; i < positions.length; i++) {
            if (positions[i] !== i) {
                throw new HttpError(
                    400,
                    "position must be 0..N-1 with no gaps",
                )
            }
        }

        await c.query(
            `UPDATE plan_exercises SET position = position + 1000 WHERE plan_day_id = $1`,
            [dayId],
        )
        for (const item of input) {
            await c.query(
                `UPDATE plan_exercises SET position = $1 WHERE id = $2`,
                [item.position, item.planExerciseId],
            )
        }
        await flipToCustomIfNeeded(c, plan.id, plan.splitType)
    })
