...
async function seed() {
    ...
    const haveExercises = await pool.query(
        `SELECT COUNT(*)::int AS n FROM exercises`,
    )
    if ((haveExercises.rows[0]?.n ?? 0) < 30) {
        throw new Error(
            "Run `npm run seed:exercises` first — the plan generator needs the catalog.",
        )
    }
    ...
    for (let i = BODY_METRIC_DAYS - 1; i >= 0; i--) {
        ...
        await pool.query(
            `INSERT INTO body_metrics (user_id, recorded_at, weight_kg, body_fat_pct)
             VALUES ($1, $2, $3, $4)`,
            [userId, dateIso, weightKg, bodyFatPct],
        )
    }
    ...
    console.log(
        `[seed] generated plan: ${plan.splitType} × ${plan.daysPerWeek} days, ${plan.days.reduce((acc, d) => acc + d.exercises.length, 0)} total exercises`,
    )
    ...
    for (let i = 0; i < NUM_SESSIONS; i++) {
        ...
        for (const pe of day.exercises) {
            ...
            const targetReps = Math.round(
                (pe.targetRepsMin + pe.targetRepsMax) / 2,
            )
            ...
            for (let setN = 1; setN <= pe.targetSets; setN++) {
                ...
                const reps = Math.max(
                    pe.targetRepsMin,
                    targetReps - Math.floor((setN - 1) / 2),
                )
    ...
    console.log(
        `[seed] inserted ${NUM_SESSIONS} sessions with ${totalSetsLogged} total sets`,
    )
