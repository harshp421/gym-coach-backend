...
let started = false;
let timer = null;
export function startEmailWorker(intervalMs = 5_000) {
    if (started)
        return;
    started = true;
    ...
    const tick = async () => {
        try {
            ...
            if (consecutiveFailures > 0) {
                console.log(`[email-worker] recovered after ${consecutiveFailures} failed tick${consecutiveFailures === 1 ? "" : "s"}`);
            }
            ...
            if (stale) {
                if (consecutiveFailures === 1) {
                    console.warn(`[email-worker] stale db connection — pool will recycle. (${message})`);
                }
            ...
            if (consecutiveFailures === 1 || lastErrorMessage !== message) {
                ...
                console.warn(`[email-worker] still failing (${consecutiveFailures} ticks): ${message}`);
            }
            lastErrorMessage = message;
        }
    };
