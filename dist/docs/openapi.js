...
export const openapiSpec = {
    ...
    info: {
        title: "Gym Coach API",
        description: "AI-assisted gym companion. Auth uses an httpOnly cookie set by `/auth/login`; every protected route reads it.",
        version: "0.1.0",
    },
    ...
    paths: {
        ...
        "/auth/forgot-password": {
            post: {
                ...
                responses: {
                    "200": {
                        description: "Always returns ok (doesn't leak whether email exists)",
                    },
                },
            },
        },
        ...
        "/profile/complete-onboarding": {
            post: {
                tags: ["profile"],
                summary: "Complete onboarding wizard (atomic profile + initial weight)",
                security: [cookieAuth],
                responses: {
        ...
        "/workouts/plan/days/{dayId}/exercises": {
            post: {
                tags: ["plan-edit"],
                summary: "Add exercise to day (provide exerciseId OR userExerciseId)",
                security: [cookieAuth],
                parameters: [
                    {
                        name: "dayId",
        ...
        "/workouts/plan/exercises/{planExerciseId}": {
            patch: {
                tags: ["plan-edit"],
                summary: "Swap or edit a plan_exercise (partial body, accepts swap-source XOR field edits)",
                security: [cookieAuth],
                parameters: [
                    {
                        name: "planExerciseId",
        ...
        "/workouts/sessions": {
            get: {
                tags: ["sessions"],
                summary: "Recent sessions (with per-session totalSets/totalReps/totalVolumeKg)",
                security: [cookieAuth],
                parameters: [
                    {
                        name: "limit",
            ...
            post: {
                ...
                responses: {
                    ...
                    "401": auth401,
                    "409": {
                        description: "Already have an in-progress session — finish or abandon first",
                    },
                },
            },
        },
        ...
        "/workouts/sessions/{id}/sets": {
            post: {
                tags: ["sessions"],
                summary: "UPSERT a set log on (sessionId, planExerciseId, setNumber)",
                security: [cookieAuth],
                parameters: [
                    {
                        name: "id",
        ...
        "/workouts/exercises/{planExerciseId}/history": {
            get: {
                tags: ["sessions"],
                summary: "Per-exercise history (cross-plan; includes Epley-based isPr flags)",
                security: [cookieAuth],
                parameters: [
                    {
                        name: "planExerciseId",
        ...
        "/coach/messages": {
            ...
            post: {
                tags: ["coach"],
                summary: "Send a message — streams assistant tokens via Server-Sent Events",
                description: "Response is `text/event-stream`: sequence of `event: chunk` (`data: {text}`), then a single `event: done` (`data: {messageId}`), or `event: error`. Returns 503 with `coach_not_configured` when GROQ_API_KEY is missing.",
                security: [cookieAuth],
                requestBody: {
                    required: true,
                    content: {
                ...
                responses: {
                    "200": {
                        description: "SSE stream. Final assistant message is persisted server-side at end of stream.",
                        content: {
                            "text/event-stream": {
                                schema: { type: "string" },
                            },
