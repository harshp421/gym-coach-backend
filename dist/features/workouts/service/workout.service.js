...
import { EQUIPMENT_ALLOWED, LEVEL_ALLOWED, generatePlan, } from "./generator.service.js";
...
function mapPlanExerciseRow(row, exercise) {
    return {
        id: row.id,
...
export async function getActivePlan(userId) {
    const planResult = await pool.query(`SELECT * FROM workout_plans WHERE user_id = $1 AND status = 'active'`, [userId]);
    const planRow = planResult.rows[0];
    if (!planRow)
        return null;
    const daysResult = await pool.query(`SELECT * FROM plan_days WHERE plan_id = $1 ORDER BY day_index ASC`, [planRow.id]);
    ...
    const peResult = await pool.query(`SELECT pe.id            AS pe_id,
...
// Generate + persist
// ---------------------------------------------------------------------------
export async function regeneratePlanForUser(userId) {
    const profile = await getOrCreateProfile(userId);
    if (!profile.experienceLevel ||
        !profile.trainingDaysPerWeek ||
        !profile.equipmentAccess ||
        !profile.goal) {
        throw new HttpError(400, "Complete onboarding before generating a plan");
    }
    ...
    if (exercises.length === 0) {
        throw new HttpError(500, "No exercises match your equipment + level — seed may be missing");
    }
...
async function persistGeneratedPlan(userId, generated) {
    const client = await pool.connect();
    try {
        await client.query("BEGIN");
        // Archive any existing active plan.
        await client.query(`UPDATE workout_plans SET status = 'archived'
             WHERE user_id = $1 AND status = 'active'`, [userId]);
        // Insert the new plan.
        const planInsert = await client.query(`INSERT INTO workout_plans
                 (user_id, split_type, days_per_week, goal)
             VALUES ($1, $2, $3, $4)
             RETURNING id`, [userId, generated.splitType, generated.daysPerWeek, generated.goal]);
        const planId = planInsert.rows[0].id;
        // Insert days + exercises.
        for (const day of generated.days) {
            const dayInsert = await client.query(`INSERT INTO plan_days (plan_id, day_index, name, weekday_hint)
                 VALUES ($1, $2, $3, $4)
                 RETURNING id`, [planId, day.dayIndex, day.name, day.weekdayHint]);
            const dayId = dayInsert.rows[0].id;
            for (const ex of day.exercises) {
                await client.query(`INSERT INTO plan_exercises (
...
// Today
// ---------------------------------------------------------------------------
export async function getTodayForUser(userId) {
    const plan = await getActivePlan(userId);
    if (!plan)
        return null;
    const todayDayIndex = new Date().getDay() % plan.daysPerWeek;
    return { plan, todayDayIndex };
}
