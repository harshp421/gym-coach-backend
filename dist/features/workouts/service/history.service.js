...
// zeroed summary.
// ---------------------------------------------------------------------------
export async function listRecentSessions(userId, opts) {
    const params = [userId];
    ...
    const result = await pool.query(`SELECT ws.*,
    ...
    const nextBefore = result.rows.length > opts.limit
...
// ---------------------------------------------------------------------------
export async function getExerciseHistory(userId, planExerciseId, limit) {
    ...
    const lookup = await pool.query(`SELECT pe.exercise_id, pe.user_exercise_id
         FROM plan_exercises pe
         JOIN plan_days pd ON pd.id = pe.plan_day_id
         JOIN workout_plans wp ON wp.id = pd.plan_id
         WHERE pe.id = $1 AND wp.user_id = $2`, [planExerciseId, userId]);
    if (!lookup.rows[0])
        throw new HttpError(404, "Exercise not found");
    const sysId = lookup.rows[0].exercise_id;
    const userExId = lookup.rows[0].user_exercise_id;
    ...
    // have to branch.
    const sessionRows = await pool.query(`SELECT DISTINCT ws.id, ws.started_at, ws.completed_at
         FROM workout_sessions ws
         ...
         ORDER BY ws.started_at DESC
         LIMIT $4`, [userId, sysId, userExId, limit]);
    if (sessionRows.rows.length === 0)
        return { items: [] };
    const sessionIds = sessionRows.rows.map((r) => r.id);
    const setRows = await pool.query(`SELECT sl.*
         FROM set_logs sl
         JOIN plan_exercises pe ON pe.id = sl.plan_exercise_id
         ...
         ORDER BY sl.logged_at ASC`, [sessionIds, sysId, userExId]);
    ...
    const items = sessionRows.rows.map((row) => ({
        sessionId: row.id,
        startedAt: row.started_at,
        completedAt: row.completed_at,
