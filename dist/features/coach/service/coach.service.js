...
import { isGroqConfigured, streamGroqChat, visionContent, } from "./groq.client.js";
...
async function getOrCreateSession(userId) {
    await pool.query(`INSERT INTO chat_sessions (user_id) VALUES ($1)
         ON CONFLICT (user_id) DO NOTHING`, [userId]);
    const r = await pool.query(`SELECT * FROM chat_sessions WHERE user_id = $1`, [userId]);
    return mapSession(r.rows[0]);
}
export async function listMessages(userId, limit) {
    ...
    const r = await pool.query(`SELECT * FROM (
         ...
         ORDER BY created_at ASC`, [session.id, limit]);
 ...
 */
export async function startTurn(userId, name, userContent, imageUrl) {
    if (!isGroqConfigured()) {
        throw new HttpError(503, "coach_not_configured");
    }
    const session = await getOrCreateSession(userId);
    await pool.query(`INSERT INTO chat_messages (session_id, role, content, image_url)
         VALUES ($1, 'user', $2, $3)`, [session.id, userContent, imageUrl ?? null]);
...
async function loadRecentTurns(sessionId, limit) {
    const r = await pool.query(`SELECT * FROM (
...
export async function persistAssistantMessage(sessionId, content) {
    const r = await pool.query(`INSERT INTO chat_messages (session_id, role, content)
         VALUES ($1, 'assistant', $2)
         RETURNING *`, [sessionId, content]);
