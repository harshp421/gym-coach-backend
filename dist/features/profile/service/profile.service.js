...
export async function updateProfile(userId, input) {
    ...
    await pool.query(`INSERT INTO profiles (user_id) VALUES ($1) ON CONFLICT (user_id) DO NOTHING`, [userId]);
    values.push(userId);
    const result = await pool.query(`UPDATE profiles SET ${setClauses.join(", ")}
         WHERE user_id = $${i}
         RETURNING *`, values);
    return mapProfile(result.rows[0]);
}
...
export async function completeOnboarding(userId, input) {
    ...
    try {
        ...
        const profileResult = await client.query(`INSERT INTO profiles (
        ...
        const metricResult = await client.query(`INSERT INTO body_metrics (user_id, recorded_at, weight_kg)
             VALUES ($1, $2, $3)
             ON CONFLICT (user_id, recorded_at) DO UPDATE SET
                 weight_kg = EXCLUDED.weight_kg
             RETURNING *`, [userId, today, input.initialWeightKg]);
...
// ---------------------------------------------------------------------------
export async function listBodyMetrics(userId, opts) {
    const params = [userId];
    ...
    const result = await pool.query(`SELECT * FROM body_metrics ${where}
...
export async function upsertBodyMetric(userId, input) {
    const recordedAt = input.recordedAt ?? new Date().toISOString().slice(0, 10);
    const result = await pool.query(`INSERT INTO body_metrics (
